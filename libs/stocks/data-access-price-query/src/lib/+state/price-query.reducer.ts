...
import { transformPriceQueryResponse, filterOutPriceQueryResponse } from './price-query-transformer.util';
  ...
  switch (action.type) {
    case PriceQueryActionTypes.PriceQueryFetched: {
      return priceQueryAdapter.addAll(
        filterOutPriceQueryResponse(action.queryResults,action.fromDate,action.toDate),
        state
      );
    }
